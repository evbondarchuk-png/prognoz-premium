...
- **24.08.2026, статус: Премиум на паузе** (решение Егора — «не работаем, не
  особо нужен»). Заметка на будущее: v1-календарь переведён на callable, в общем
  бэке появились `editTask`/`reopenTask`/`deleteTask` (см. `prognoz-functions/
  CLAUDE.md` §8). ⚠️ Премиум-копия `calendar.js` всё ещё пишет в `/tasks`
  напрямую — при реактивации Премиума перевести её на callable тем же способом,
  как в v1 (`prognoz-dashboard/TASKS-SYSTEM.md` §10).
